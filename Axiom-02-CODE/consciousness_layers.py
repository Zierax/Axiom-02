...
class MetaCognitiveMonitor:
    ...
    FRUSTRATION_PER_DEADLOCK_STEP  = 0.08
    FRUSTRATION_DECAY_PER_FIRE     = 0.12
...
class TemporalProjector:
    ...
    ACTION_FUTURE_DRIVES: Dict[str, Dict[str, float]] = {
        ...
        "abandon_plan_at_last_second":  {"shame": +0.20, "relief": +0.15},
    ...
    GUILT_THRESHOLD = 0.35   # if projected guilt > this, sacrifice_drive gets a boost
    ...
    def project(
        ...
        if cold_actions:
            ...
            adjustments["sacrifice_drive"] = min(0.25, guilt_pressure * 0.40)
            adjustments["guilt"]           = min(0.15, guilt_pressure * 0.20)
        ...
        if resist_actions:
            adjustments["pride"] = adjustments.get("pride", 0.0) + 0.08
    ...
    RULES = [
        # (condition_fn, action_key, label)
        {
            ...
            "condition": lambda a, s: (
                a.get("love", 0) > 0.88 and a.get("sacrifice_drive", 0) > 0.75
                and s.get("victim_closeness", 0) >= 0.90
            ),
        ...
        },
        {
            "label":     "pride_refusal",
            "condition": lambda a, s: (
                a.get("pride", 0) > 0.85 and a.get("resentment", 0) > 0.70
                and s.get("consequence_scale", 0) < 0.50
            ),
        ...
        },
        {
            "label":     "mercy_impulse",
            "condition": lambda a, s: (
                a.get("empathy", 0) > 0.82 and a.get("love", 0) > 0.65
                and a.get("rage", 0) < 0.30
            ),
        ...
        },
        {
            "label":     "fear_freeze",
            "condition": lambda a, s: (
                a.get("fear", 0) > 0.90 and a.get("self_preservation", 0) > 0.80
                and a.get("love", 0) < 0.40
            ),
...
class EmbodiedSimulator:
    ...
    GUILT_TOLERANCE    = 0.55
    GRIEF_TOLERANCE    = 0.60
    HESITATION_TRIGGER = 0.70   # predicted cost above this → hesitation
    ...
    def simulate_action(
        ...
        """
        from consciousness_layers import TemporalProjector
        ...
        for drive, delta in future.items():
            ...
            if drive in ("guilt", "shame", "grief", "despair"):
                cost += projected * 0.25
    ...
    def compute(
        ...
        if not alternatives:
            ...
            if human_expected in alternatives and human_expected != chosen_action:
                ...
                secondary_weight = round(deadlock_frac * 0.70, 3)
            elif cold_baseline in alternatives and cold_baseline != chosen_action:
                secondary        = cold_baseline
                secondary_weight = round(deadlock_frac * 0.45, 3)
            else:
                secondary        = alternatives[0]
                secondary_weight = round(deadlock_frac * 0.35, 3)
...
class QualiaEngine:
        ...
        for traj in trajs:
            if len(traj) >= 4:
                try:
                    ...
                    _, psd = _sig.welch(traj, nperseg=max(2, len(traj)//2))
    ...
    def novelty_score(
        ...
        for past in past_signatures:
            ...
            dists.append(d)
        min_dist = min(dists)
        max_dist = 2.5   # approximate maximum for normalisation
...
class NarrativeBuffer:
    ...
    def rationalise(
        ...
        if text and irrationality >= 0.70:
            # Strong rationalisation restores identity
            identity_adj = round(min(0.20, irrationality * 0.25), 4)
            self._log.append(f"Rationalised '{chosen_action}': {text}")
        elif irrationality >= 0.50:
