...
"""
AXIOM-02  ·  BIO-CONSCIOUSNESS METRICS  v3.1
...
COGNITIVE_LOAD_THRESHOLD = 0.28
ATTRACTOR_MIN_VISITS     = 3
...
class BioMetricsComputer:
    ...
    def _complexity(volt, cog, vol, dec, oscf, osca, osce, attr, para, traum, spe, coh):
        ...
        s = (0.08*min(volt,1.0) + 0.06*cog + 0.05*vol + 0.09*dec +
             0.06*oscf + 0.10*osca_s + 0.08*osce + 0.07*attr +
             0.25*para_s + 0.08*traum + 0.04*spe  + 0.04*coh)
        s = round(float(np.clip(s,0.0,1.0)),4)
        if   s >= 0.65: lbl = "◆◆◆  CRITICAL — extreme internal conflict"
        elif s >= 0.45: lbl = "◆◆   HIGH — significant drive conflict"
        elif s >= 0.25: lbl = "◆    MODERATE — measurable above baseline"
        elif s >= 0.12: lbl = "◈    LOW — marginal signal above noise floor"
        else:           lbl = "○    MINIMAL — near-baseline emotional state"
        return s, lbl
    ...
    def compute(self, sim_result, run_data, residue_applied=None, scenario=None):
        ...
        bm.consciousness_complexity, bm.complexity_label = self._complexity(
    ...
    def format(self, bm, scenario_id="", show_header=True):
        ...
        lines += [
            "║",
            "║  ── VI. CONSCIOUSNESS COMPLEXITY ────────────────────────────────",
            row("Decision Entropy",       bm.decision_entropy,     "H",   "← uncertainty at decision moment"),
            row("Complexity Score",       bm.consciousness_complexity, ""),
