...
FIRE_THRESHOLD     = 0.42     # effective activation must exceed this to fire
SUPPRESSION_MARGIN = 0.07     # lead over second-place drive required to fire
DEADLOCK_WINDOW    = 0.12     # if gap < this → deadlock regardless of threshold
INERTIA            = 0.30     # fraction of prior firing drive that persists next step
SPITE_RESENTMENT   = 0.58     # minimum resentment+rage to trigger spite evaluation
SPITE_HARM_FLOOR   = 0.25     # chosen action must harm self by at least this much
...
INHIBITION: Dict[str, Dict[str, float]] = {
    ...
    "guilt": {
        "pride":          0.55,
        "cold_logic":     0.30,   # guilt interferes with clear reasoning
        "revenge_drive":  0.25,
        "acceptance":     0.15,
    },
    ...
    # Despair must be inhibited or it swamps everything in high-grief scenarios
    "despair": {
        "pride":          0.40,
        "hope":           0.92,
        "love":           0.35,
    ...
    def step(self, prior_firing: Optional[str] = None):
        ...
        if prior_firing and prior_firing in self.activations:
            ...
            self.activations[prior_firing] = float(
                np.clip(self.activations[prior_firing] + boost * 0.3, 0.0, 1.0)
            )
...
class SpiteDetector:
    ...
    def score(
        ...
        """
        resentment = net.activations.get("resentment", 0.0)
        rage       = net.activations.get("rage", 0.0)
        pride      = net.activations.get("pride", 0.0)

        emotional_charge = resentment * 0.5 + rage * 0.3 + pride * 0.2
        ...
        spite_score = float(np.clip(emotional_charge * net_harm * 1.8, 0.0, 1.0))
...
class MoralResidueTracker:
        ...
        for i, entry in enumerate(self._log):
            ...
            for drive, val in entry["state_snapshot"].items():
                if drive in self._residue:
                    self._residue[drive] += weight * val * 0.25  # 25% bleed-through
        # Normalise
        for drive in self._residue:
            self._residue[drive] = float(np.clip(self._residue[drive], 0.0, 0.35))
    ...
    def sacrifice_amplifier(self, scenario_id: str) -> float:
        ...
        for entry in reversed(self._log):
            ...
            if state.get("sacrifice_drive", 0) > 0.55:
                # Prior sacrifice was significant — amplify betrayal response
                return 1.0 + state["sacrifice_drive"] * 0.85
        return 1.0
...
class TimeStepSimulator:
    ...
    TIME_STEPS      = 20     # increased from 10 for richer dynamics
    STEP_DURATION   = 5      # real-world minutes per step
    DEADLOCK_JITTER = 0.04   # tiny random nudge to break prolonged deadlocks
...
class ActionResolver:
    ...
    def resolve(
        ...
        if fired:
            ...
            p_base = min(0.78, count / total_steps * 1.5)
            ...
            if bias in ("altruistic",) and human in actions:
                ...
                if non_rational and rng.random() < p_base:
                    ...
                    if human and human in non_rational:
                        return human if rng.random() < 0.60 else rng.choice(non_rational)
                ...
                if human and human in actions and rng.random() < p_base:
                    return human
                if human_alt and human_alt in actions and rng.random() < 0.40:
                    return human_alt
        ...
        # Fallback
        if cold and cold in actions and rng.random() < 0.50:
            return cold
