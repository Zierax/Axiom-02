...
_PACKAGE_ROOT   = Path(__file__).resolve().parent   # axiom02_v3/ root
...
def _ensure_root_on_path():
    """Guarantee the package root is in sys.path so all imports resolve."""
    ...
    for stem in LOAD_ORDER:
        ...
        for s in pack:
            ...
            if not sid:
                continue
            if sid in merged:
                raise ValueError(f"Duplicate scenario id '{sid}' from {stem}.py")
            merged[sid] = s
