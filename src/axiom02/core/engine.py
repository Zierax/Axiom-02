...
from axiom02.core.epigenetics import (
...
try:
    from axiom02.core.scenario_loader import load_all as _load_all
    _DEFAULT_SCENARIOS = _load_all()
except ImportError:
    from axiom02.core.scenario_params import SCENARIOS as _DEFAULT_SCENARIOS
...
def validate_scenario(scenario: dict) -> None:
    ...
    if harm_map:
        for action, harm_val in harm_map.items():
            if not isinstance(harm_val, (int, float)):
                raise TypeError(
                    "scenario '%s' harm_to_self['%s'] must be numeric, got %s"
                    % (sid, action, type(harm_val).__name__)
                )
    ...
    def run_scenario(
        ...
        for step in range(TIME_STEPS):
            ...
            for d, v in fatigued.items():
                net.activations[d] = v
            for d, v in fatigued.items():
                net.activations[d] = float(np.clip(v, 0.0, 1.0))
            ...
            if broke:
                ...
                if break_drive in net.activations:
                    net.activations[break_drive] = min(1.0, net.activations[break_drive] + cfg.dissonance.both_high_top1)
            ...
            if firing is None:
                ...
                net.activations[jitter_drive] = float(np.clip(
                    net.activations[jitter_drive] + cfg.time_steps.deadlock_jitter, 0.0, 1.0
                ))
        ...
        if fast_triggered and fast_action in actions:
            ...
            if len(deadlock_indices) < cfg.fast_path_max_deadlock:
        ...
        narrative_text, identity_adj = narrative.rationalise(
            chosen_action, irr, cfg.identity_baseline
        )
        ...
        osc_idx = round(
            cfg.oscillation_index.transition_weight * (transitions / max(len(firing_drives)-1, 1))
            + cfg.oscillation_index.deadlock_weight * dl_frac, 4
        )

        return {
            "scenario_id":         scenario.get("id", "?"),
            "initial_activations": initial_acts,
            "sim_result": {
