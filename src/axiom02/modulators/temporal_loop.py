...
__all__ = [
    ...
    "ALPHA_CIRC",
    "ALPHA_RUMI",
    "CORTISOL_SE_THRESHOLD",
    "CORTISOL_SE_KAPPA",
    "NOREPI_ENTROPY_SCALE",
    "NARRATIVE_STABILITY_DECAY",
    "NARRATIVE_RECOVERY_COEFF",
    "NARRATIVE_EROSION_IRR",
    "NARRATIVE_EROSION_SPITE",
    "NARRATIVE_EROSION_DL",
    "MODULATOR_COUPLING",
    "MODULATOR_BASELINES",
...
ALPHA_PERSIST:   float = cfg.temporal_loop.alpha_persist
ALPHA_CIRC:      float = cfg.temporal_loop.alpha_circ
ALPHA_RUMI:      float = cfg.temporal_loop.alpha_rumi

CORTISOL_SE_THRESHOLD: float = cfg.temporal_loop.cortisol_se_threshold
CORTISOL_SE_KAPPA:     float = cfg.temporal_loop.cortisol_se_kappa

NOREPI_ENTROPY_SCALE:  float = cfg.temporal_loop.norepi_entropy_scale

NARRATIVE_STABILITY_DECAY: float = cfg.temporal_loop.narrative_stability_decay
NARRATIVE_RECOVERY_COEFF:  float = cfg.temporal_loop.narrative_recovery_coeff
NARRATIVE_EROSION_IRR:     float = cfg.temporal_loop.narrative_erosion_irr
NARRATIVE_EROSION_SPITE:   float = cfg.temporal_loop.narrative_erosion_spite
NARRATIVE_EROSION_DL:      float = cfg.temporal_loop.narrative_erosion_dl

MODULATOR_COUPLING: Dict[Tuple[str, str], float] = cfg.modulator_coupling

MODULATOR_BASELINES: Dict[str, float] = cfg.temporal_modulator_defaults
...
class ModulatorPropagator:
    ...
    def propagate(
        ...
        """
        mods: Dict[str, float] = {
            "dopamine":       scenario_mods.get("dopamine",       cfg.modulator_baseline.dopamine),
            "serotonin":      scenario_mods.get("serotonin",      cfg.modulator_baseline.serotonin),
            "norepinephrine": scenario_mods.get("norepinephrine", cfg.modulator_baseline.norepinephrine),
            "cortisol":       scenario_mods.get("cortisol",       cfg.modulator_baseline.cortisol),
            "oxytocin":       scenario_mods.get("oxytocin",       cfg.modulator_baseline.oxytocin),
        }
        ...
        if propagated["cortisol"] > CORTISOL_SE_THRESHOLD:
            ...
            new_cortisol_hours  = prior.cortisol_hours * math.exp(cfg.temporal_cortisol_hours_decay * hours_elapsed)
        ...
        new_serotonin_depl = float(np.clip(
            ...
            0.0, cfg.temporal_serotonin_depletion_cap
        ))
    ...
    def adjust_decision_entropy(
        ...
        """
        suppression = NOREPI_ENTROPY_SCALE * max(0.0, mods.norepinephrine - cfg.temporal_norepi_entropy_threshold)
...
class TemporalEmotionLoop:
    ...
    def _run_one_step(
        ...
        for mod in forced_mods:
            if mod in run_mods:
                run_mods[mod] = round(
                    run_mods[mod] * cfg.temporal_scenario_blend + forced_mods[mod] * cfg.temporal_prior_blend, 4
                )
        ...
        fatigue = self._circadian.accumulate_fatigue(cfg.temporal_fatigue_accumulation)
        ...
        adj_entropy = self._propagator.adjust_decision_entropy(cfg.temporal_base_entropy, new_mods)
