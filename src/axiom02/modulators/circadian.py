...
class CircadianEngine:
    ...
    def __init__(self, hours_per_scenario: float = 1.0, max_log_size: int = 500) -> None:
    ...
    def accumulate_fatigue(self, cognitive_load_pct: float) -> float:
        ...
        if len(self._fatigue_log) > self._max_log_size:
            self._fatigue_log = self._fatigue_log[-self._max_log_size:]
    ...
    def advance(self, hours: Optional[float] = None) -> None:
        ...
        if len(self._snapshot_log) > self._max_log_size:
            self._snapshot_log = self._snapshot_log[-self._max_log_size:]
